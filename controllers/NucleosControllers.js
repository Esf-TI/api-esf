...
const CreateNucleo = async (req, res) => {
  ...
  try {
    ...
    const inserirNucleo =
      "INSERT INTO Nucleo (Nome, Email, Senha, Cidade, Descricao, DataFundacao, fotoCapa, linkDoacao, linkSite, linkLinkedin, linkFacebook, linkInstagram, status, subdominio) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    connection.query(
      inserirNucleo,
      [
        nomeNucleo,
      ...
      async (err, result) => {
        ...
        return res
          .status(200)
          .send({ id: nucleoId, message: "Núcleo criado com sucesso!" });
      }
    );
  } catch (error) {
    console.log(error);
        ...
        connection.query(
          ...
          (err, insertResult) => {
            if (err) {
              ...
              return res
                .status(500)
                .send("Erro ao salvar os tokens no banco de dados");
            }
  ...
  connection.query(getNucleoNomeQuery, [nucleoId], (error, results) => {
    ...
    connection.query(
      ...
      async (error, results) => {
        ...
        console.log(
          `Status do núcleo ${nucleoNome} atualizado para ${novoStatus}`
        );
        ...
        if (novoStatus === "approved" && subdominio === null) {
          try {
            ...
            const updateSubdomainQuery =
              "UPDATE Nucleo SET subdominio = ? WHERE ID = ?";
            connection.query(
              updateSubdomainQuery,
              [newSubdomain, nucleoId],
              (error, results) => {
                if (error) {
                  console.error(
                    "Erro ao atualizar subdomínio do núcleo: " + error.message
                  );
                  res
                    .status(500)
                    .send("Erro ao atualizar subdomínio do núcleo");
                  return;
                }

                console.log(
                  `Subdomínio do núcleo ${nucleoNome} atualizado para ${newSubdomain}`
                );
                res
                  .status(200)
                  .send(
                    `Status do núcleo ${nucleoNome} atualizado para ${novoStatus} e subdomínio criado como ${newSubdomain}`
                  );
              }
            );
          } catch (error) {
...
const patchNucleo = (req, res) => {
  ...
  if (!nucleoId || !campoAAlterar || !novoValor) {
    res
      .status(400)
      .send(
        "O ID do núcleo, o campo a ser alterado e o novo valor são obrigatórios"
      );
    return;
  }
  ...
  connection.query(checkNucleoQuery, [nucleoId], (error, results) => {
    ...
    connection.query(
      ...
      (error, results) => {
        ...
        console.log(
          `Campo ${campoAAlterar} do núcleo ${nucleoId} atualizado com sucesso!`
        );
        res
          .status(200)
          .send(
