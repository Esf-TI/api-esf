...
// O Supabase Storage rejeita chaves com caracteres não-ASCII (acentos, ç, etc.).
// Pastas derivadas de nomes em PT (ex.: "Conselho de Administração", "Prestação de
// Contas") quebravam o upload. Normaliza cada segmento para um slug ASCII seguro.
  ...
  String(input || "")
    ...
    .map((seg) =>
      seg
        ...
        .toLowerCase()
        .replace(/[^a-z0-9._-]+/g, "-") // mantém só caracteres seguros
        .replace(/-+/g, "-")
        .replace(/(^-|-$)/g, "")
    )
    .filter(Boolean)
...
const buildObjectPath = (folder, originalName, newExt) => {
  ...
  return `${slugifyPath(folder)}/${Date.now()}-${randomSuffix()}${ext}`
}
